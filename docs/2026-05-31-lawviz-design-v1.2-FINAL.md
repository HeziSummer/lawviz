...
**产品定位**：律析 LawViz — 律师用 AI 生成案件可视化 HTML 报告的 Web SaaS，灰测版给朋友玩，正式版走**订阅 + 按量计费**的二元模式。

**核心改变**（vs v1.1）：
1. **Tier 定义**：从"服务访问权"改为"**月度订阅套餐**"——用户选一个月度会员（基础/进阶/专业），获得该月积分额度+权益，用完后可单次充值。
2. **灰测计费**：所有价格用"**占位价**"（灰测期临时，不是最终定价），确保支付/积分链路可验证，正式版再用真实成本校准。
